...
$(document).ready(function(){
	...
	$("#bttn_generar_op").on('click',function(){
		...
		$.ajax({
			...
			success:function(data){
				alert(data);
				window.open("pdf_op.php?op="+data+"",'_blank');
				win.focus();
			}
		});		
	});
...
function ocultar_resumen_ppto(){
	if($(".ocultar_opciones_ppto").is(":visible")){
		$("#opcion_para_ocultar_resumen").removeClass("triangulo_inf");
		$("#opcion_para_ocultar_resumen").addClass("triangulo_sup");
		$("#contenedor_ppto_x").css({"height":"640px","overflow-y":"hidden"});
		$("#contenedor_resumen_ppto").css({"height":"auto"});	
		$(".ocultar_opciones_ppto").hide();
	}else{
		$("#opcion_para_ocultar_resumen").addClass("triangulo_inf");
		$("#opcion_para_ocultar_resumen").removeClass("triangulo_sup");
		$("#contenedor_ppto_x").css({"height":"440px"});
		$("#contenedor_resumen_ppto").css({"height":"230px","overflow-y":"scroll"});	
		$(".ocultar_opciones_ppto").show();
	}
}