<?php
	...
	while($row = mysql_fetch_array($sql)){
		...
		for($i = 0;$i < count($ee)-1;$i++){
			$info = explode("<***+++>",$ee[$i]);
			$list_temas.='<tr><td flogo = "border:1px solid black;width:20%;border:0px;padding-left:10px;">'.nl2br($info[0]).'</td></tr>';
		}
	...
	$headerHTML='<div class="logo"><img src = "../images/logos/'.$logo.'" height = "80px"/>'.$row['name'].'</div>';
