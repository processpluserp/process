<?php
	...
	mysql_query("START TRANSACTION");
		...
		cliente,val_desde_item,por_prov,usuario,fecha_registro,ppto,vi,vc
		from itempresup where id = '".$_POST['id']."'");
			...
			$estructura_editable = "<tr class = '$id informacion_guardada_bd item_total$i items_ppto_pro'>
								...
								<td class = 'border_table' align = 'center' style = 'background-color:#EF8B8B;color:white;' nowrap>
									<table width = '100%'>
										<tr>
											<td>$</td>
											<td align = 'right'><input type = 'text' onblur = 'limpiar_valor_costo_cliente($i);' style = 'width:80px;' class = 'entrada_bordes' value = '".number_format($row['cliente'])."' id = 'valor_costo_unitario_cliente$i' onkeyup = 'formatear_valor_costo_cliente($i);calcular_interno($i);'/></td>
										</tr>
									</table>
